...
func (h *Handlers) List(w http.ResponseWriter, r *http.Request) {
	...
	views, err := h.reader.List(r.Context(), namespace)
	if err != nil {
		httpx.Error(w, r, err)
		return
	}
	items := make([]ApplicationResponse, 0, len(views))
	for _, v := range views {
...
func (h *Handlers) Get(w http.ResponseWriter, r *http.Request) {
	view, err := h.reader.Get(r.Context(), chi.URLParam(r, "namespace"), chi.URLParam(r, "name"))
	if err != nil {
		httpx.Error(w, r, err)
		return
	}
